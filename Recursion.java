...
public final class Lab2 {
	...
	public static String interlace(String first, String second, int n) {
		...
		if(n%2 == 0) {
			x = interlace(first, second, n-1) + second ;
		}
		
		else if (n%2 == 1) {
			x =  interlace(first, second, n-1) + first ;
		}
	...
	public static String getSubstring(String str, char open, char close) {
		...
		if(open == str.charAt(0) && close == str.length()-1){
			return x;
		}
		
		else if (str.charAt(0) != open){
			x = getSubstring(str.substring(1), open, close);
		}
		
		else if(str.charAt(str.length()-1) != close) {
			x = getSubstring(str.substring(0, str.length()-1), open, close);
		}
...
//
//
//	public TicketingSystem(int mulanSeats, int avengersSeats, int tenetSeats) {
//		this.m = mulanSeats;
//		this.a = avengersSeats;
//		this.t = tenetSeats;
//	}
//
//	public boolean checkSeat(int movieType) {
//		boolean flag = false;
//		int seats;
//
//		if(movieType == 1){
